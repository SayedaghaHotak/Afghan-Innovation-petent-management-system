...
const CommitteeLayout = ({ userProfile }) => { // 👁️ دریافت پراپس پروفایل
  ...
  const menuItems = [
    ...
    { id: 'assigned', label: 'Assigned Ideas', path: '/committee_dashboard/assigned' },
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8081/api/v1.0/notifications/committee', {
  ...
  return (
    <div className="committee-layout-container">
      ...
      <nav className="committee-navbar">
        ...
        <div className="navbar-brand-zone" onClick={() => navigate('/committee_dashboard/home')}>
          <FaAward className="brand-icon-orange" />
          <span className="brand-text">AIMS <small>Committee</small></span>
        </div>
        ...
        <div className="navbar-actions-zone">
          ...
          <div className="notification-bell-wrapper" onClick={() => setShowNotifDropdown(!showNotifDropdown)}>
          ...
          <div className="user-profile-meta">
            <div className="user-avatar-circle">
              {currentProfile.avatar ? <img src={currentProfile.avatar} alt="User" className="nav-avatar-img" /> : currentProfile.name.charAt(0).toUpperCase()}
            </div>
